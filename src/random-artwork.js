...
var RandomArtworkContainer = (ComposedComponent, options = {}) =>
  React.createClass({
    getInitialState() {
      return {
    ...
    setArtwork(id) {
      ...
      if (preloadNext && this.state.next) {
        ...
        rest(searchUrl).then((data) =>
          this.setState({ art: JSON.parse(data.entity) })
        );
      }
      if (preloadNext)
        rest(searchUrl).then((data) =>
          this.setState({ next: JSON.parse(data.entity) })
        );
    },
