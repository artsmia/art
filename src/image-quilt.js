...
const ImageQuilt = React.createClass({
  ...
  render() {
    ...
    const shuffledArtworks =
      this.state.shuffledArtworks &&
      this.state.shuffledArtworks.slice(0, this.props.maxWorks);
    const _art = artworks.map((art) => {
      var s = art._source;
      if (
        s.image == "invalid" ||
        (s.image_width == 0 && s.image_height == 0) ||
        s.rights_type == "Permission Denied"
      ) {
        s.image = "invalid";
        var splitOn = /;|:|\]/;
    ...
    isInspiredByMia
      ? _art.map(
          (art) =>
            (art.aspect_ratio =
              (art.image_width + Number(hPaddingInt) * 2) / art.image_height)
        )
      : _art.map(
          (art) => (art.aspect_ratio = art.image_width / art.image_height)
        );
    const summedAspectRatio = _art.reduce((sum, art) => {
    ...
    var numRows = Math.min(
      this.props.maxRows,
      Math.max(
        Math.floor((summedAspectRatio * rowHeight) / this.state.width),
        1
      )
    );
    ...
    const images = rows.map((row, index) => {
      ...
      var images = row.map((art) => {
        ...
        const id = _art.id;
        const width =
          (_art.aspect_ratio / rowSummedAspectRatio) * this.state.width;
        const height = width / _art.aspect_ratio;
        const maxRowHeight = this.props.maxRowHeight || 200;
        const widthAdjustedToClipTallRows =
      ...
      return (
        <div className="quilt-row-wrap" key={"row" + index} style={rowStyle}>
          {images}
        </div>
      );
    });
    ...
    return (
      ...
      >
        ...
        <div id="quilt-controls" style={{ float: "right", display: "none" }}>
          ...
          {this.state.maxWorks > 0 && (
            <a onClick={this.enlarge.bind(this, -1, -10)}> / fewer</a>
          )}
          <br />
          <a onClick={this.enlarge.bind(this, 1, 0)}>bigger</a>
          <a onClick={this.enlarge.bind(this, -1, 0)}> / smaller</a>
          <br />
          {this.state.morphDelta > 0 || (
            <a onClick={this.morph.bind(this, 1)}>grow!</a>
          )}
          {this.state.morphDelta < 0 || (
            <a onClick={this.morph.bind(this, -1)}> / shrink!</a>
          )}
          {this.state.morphDelta != 0 && (
            <a onClick={this.resetMorph}> / stop!</a>
          )}
          <br />
          {(this.props.maxRows == this.state.maxRows &&
            this.props.maxWorks == this.state.maxWorks) || (
  ...
  getPartition(artworks, numRows) {
    var memoKey = `rows:${this.props.maxRows}|width:${
      this.state.width
    }|${artworks.map((art) => art._id).join("-")}`;
    ...
    var rows = partitionBy(
      artworks,
      (art) => art._source.aspect_ratio * 100,
      numRows
    );
  ...
  clicked(art, updateState = true) {
    ...
    const sameArt =
      (this.state.unpinned && !updateState) ||
  ...
  emptyQuiltToggleControl() {
    ...
    return (
      <span
        ...
        style={styles}
        onDoubleClick={this.freeTheQuilt}
      />
    );
  },
...
ImageQuilt.getImagedResults = (hits) =>
  hits &&
  hits.filter(
    (hit) => hit._source.image == "valid" && hit._source.image_width > 0
  );
