...
var Search = React.createClass({
  ...
  componentWillReceiveProps(nextProps) {
    ...
    var nextTerms =
      (nextProps.params && nextProps.params.terms) || nextProps.terms;
    if (this.normalizeTerms() != nextTerms) {
      this.setState({ terms: nextTerms });
    }
    const results =
      nextProps.results || (nextProps.data && nextProps.data.searchResults);
  ...
  updateFromQuilt(art) {
    ...
    if (art) {
      ...
      if (hits.indexOf(art) === -1) {
        ...
        if (sameArtDifferentObject)
          index = hits.indexOf(sameArtDifferentObject[0]) + 1;
      }
  ...
  linkToClickedArtwork(facet, art) {
    ...
    this.transitionTo(facet ? "filteredSearchResults" : "searchResults", {
      terms: "*",
      splat: facet,
    });
  },
