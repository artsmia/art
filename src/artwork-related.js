...
var ArtworkRelatedContent = React.createClass({
  render() {
    ...
    var links = Object.keys(art)
      .filter((key) => key.match("related:"))
      .reduce((relateds, key) => {
        var linkOrListOfLinks =
          typeof art[key] == "string" ? JSON.parse(art[key]) : art[key];
    ...
    var sortedExploreContent = R.sortBy(
      ({ type }) =>
        ({
          audio: 17,
          video: 13,
    ...
    var groupedRelatedContents = R.mapObjIndexed((relateds, type) => {
      ...
      const showType =
        true ||
        ["audio", "video", "artstory", "3d", "inspiredByMia"].indexOf(type) < 0;
    ...
    var meat = (
      <div className="explore">{R.values(groupedRelatedContents)}</div>
    );
    var exhibition_wrap = (
    ...
    return (
      <div>
        ...
        {significantExhibitions &&
          significantExhibitions.length > 0 &&
          exhib_bones}
      </div>
    );
  },
  ...
  build(json) {
    ...
    var trackRelatedClick = this.trackRelatedContentInteraction.bind(
      this,
      json
    );
    ...
    return (
      <div onClick={trackRelatedClick} key={json.link || json.title}>
        {template.bind(this)(
          json,
          this.props.id,
  ...
  templates: {
    ...
    newsflash: (json) => (
      <div
        className="newsflash"
        style={{
          backgroundImage1: `url(https://newsflash.dx.artsmia.org${json.image})`,
        }}
      >
        <a href={json.link}>
          {json.title}
    ...
    artstory: (json, id, highlight) => (
      <div
        className={cx("artstory", {
          "block-highlight": highlight && highlight["related:artstories"],
        })}
      >
        <a href={json.link}>
          <p>
    ...
    "adopt-a-painting": (json, id, highlights) => {
      ...
      if (
        !(
          highlight ||
          (window.location && window.location.pathname.match(/related.*adopt/))
        )
      )
        return <span />;
      ...
      if (json.adopted === "1") {
        return (
          <div className="adopt-ptg" style={{ clear: "both" }}>
            <h3>This painting has been adopted.</h3>
            <a href="/info/adopt-a-painting">
        ...
        return (
          <div className="adopt-ptg">
            <div style={{ width: "100%" }}>
              <h3>Adopt-a-Painting</h3>
              <p>
                Cost <span className="cost">{json.cost}</span>
              </p>
              <Markdown>
    ...
    default: (json, id, highlights) => {
      var miaStoryMatches =
        ...
        highlights["related:stories"].filter((h) => {
          var h = JSON.parse(h);
          return json.title == h.title.replace(/<\/?em>/g, "");
        });
      var defaultStyle = {};

      return (
        <div className="explore-content" style={defaultStyle}>
          <div
            className={cx({
              "block-highlight": miaStoryMatches && miaStoryMatches.length > 0,
            })}
          >
            <a href={json.link}>
              <p>{json.title}</p>
    ...
    exhibition: (json, id, highlights) => {
      ...
      return (
        <ul className="exhibitions" style={{ clear: "both" }}>
          <li>
            <h4>
              <Link to="exhibition" params={{ id: json.id }}>
                {json.title}
              </Link>
            </h4>{" "}
            {json.date}
    ...
    video: (json, id) => {
      var iframe = (
        <iframe
          src={`https://player.vimeo.com/video/${json.link.match(/\d+/)[0]}`}
          frameborder="0"
      ...
      return (
        <div>
          <Link to="sandboxArtwork" params={{ id: json.wpid }}>
            <img src={json.image} style={{ maxWidth: "13em" }} />
            <br />
