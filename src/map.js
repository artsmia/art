...
var Map = React.createClass({
  ...
  render() {
    ...
    var floor =
      this.state.floor || this.props.floor || parseInt(parseInt(number) / 100);
    var svg = (
  ...
  handleMapClick(event) {
    ...
    if (text) {
      ...
      if (266 <= parseInt(gallery) && parseInt(gallery) <= 274)
        gallery = "266-G274";
  ...
  handleSvgLoad(event) {
    ...
    var activeText = Array.from(allTexts).filter(
      (t) => t.textContent == this.props.number
    );
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevHovered != hoveredPoly && prevHovered != activePoly)
      this.resetGallery(prevHovered);
  ...
  render() {
    ...
    var floorControls = [3, 2, 1].map((_floor) => {
      ...
      return (
        <button
          style={buttonStyle}
          onClick={this.props.handleChange.bind(this.props.map, _floor)}
          key={_floor}
        >
          {text}
        </button>
    ...
    return (
      <div style={controlStyle}>
        ...
        {!this.props.startOpen && (
          <button style={buttonStyle} onClick={this.props.closeMap}>
            close map
          </button>
        )}
      </div>
