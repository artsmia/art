...
var Artwork = React.createClass({
  ...
  render() {
    ...
    var zoomLoadedSuccessfully =
      zoomLoaded && zoomLoaded !== "error" && zoomLoadComplete;
    ...
    var map = (
      <div
        ...
        className={[
          "leaflet-container",
          zoomLoadedSuccessfully && "zoomLoaded",
        ].join(" ")}
      >
        {this.state.has3d && (
        ...
        {(art.image == "valid" && rights !== "Permission Denied" && (
          <div
            id="staticImage"
            className={zoomLoadedSuccessfully && "zoomLoaded"}
          >
            {image}
            {!!zoomLoaded ||
              (art.image_copyright && (
    ...
    var info = (
      <div className="info">
        {this.props.children || (
          <div>
            {smallViewport && (
              <div style={{ margin: "-2em 0 1em 0" }}>{detailCropper}</div>
            )}
            <ArtworkPreview
              art={art}
  ...
  initView() {
    ...
    this.setState({
      zoomLoaded: false,
      zoomLoadComplete: false,
      zoomLoadedSuccessfully: false,
    });

    if (
      art.image === "valid" &&
      this.state.showBiggie &&
      !this.isLoan() &&
      !this.state.show3d
    ) {
  ...
  loadZoom(options = { reset: false }) {
    ...
    rest(iiif0Url)
      ...
      .then((data) => {
        ...
        const showZoomControlsIfDesktop =
          !L.Browser.touch || window.innerWidth > 780;
        if (showZoomControlsIfDesktop)
        ...
        this.tiles = L.tileLayer.iiif(iiifUrl, {
          attribution: art.image_copyright
            ? decodeURIComponent(art.image_copyright)
            : "",
          fitBounds: true,
          setMaxBounds: true,
        ...
        false &&
          setTimeout(function () {
            ...
            var tileContainer = _map._mapPane.querySelector(
            ...
            if (tileElems <= 1) {
              console.info("zooming out then back in", {
                max: _map.getMaxZoom(),
              });
          ...
          if (smallViewport && !recentlyAutoZoomed) {
            ...
            var reasonsToChangeFullscreen = [
              event.type == "zoomstart" &&
                zoomInCount == 2 &&
                !this.map.isFullscreen(), // zooming in when not fullscreen
              event.type == "zoomend" &&
                nowZoom >= 3 &&
                !this.map.isFullscreen(), // not fullscreen and zoomed in quite far
              event.type == "zoomend" &&
                this.map.getZoom() == this.map.getMinZoom() &&
                this.map.isFullscreen(), // in fullscreen, zooming out to minZoom
            ];
            if (reasonsToChangeFullscreen.find((reason) => reason)) {
              ...
              this.zoomCountResetTimer = setTimeout(() => {
                zoomCount = zoomInCount = zoomOutCount = 0;
                recentlyAutoZoomed = false;
              }, 1000);
            }
          }
        });
  ...
  captureViewRegion() {
    ...
    let region = !constrainToAspectRatio
      ? [x0, y0, w, h]
      : [adjustedX0, adjustedY0, adjustedW, adjustedH];
  ...
  imageStatus() {
    ...
    var copyrightAndOnViewMessage =
      art.room && art.room[0] == "G"
        ? " (You'll have to come see it in person.)"
        : "";
    var loadingZoomMessage = `
      (—Is that the best image you've got!!?
      —Nope! We're loading ${humanizeNumber(
        this.getPixelDifference(800)
      )} more pixels right now.
      It can take a few seconds.)`;
    var showLoadingMessage =
      zoomLoading && zoomLoaded === false && !this.context.universal;
        ...
        <span className="imageStatus">
          ...
          {showErrorMessage && (
            <p>
              ...
              <a href="mailto:collectionsdata+images@artsmia.org">
                Report this problem
              </a>
              .
            </p>
...
var IIIFCropper = React.createClass({
  ...
  render() {
    ...
    const setSquareSmartCrop = () =>
      updateRegion({ iiifRegion: squareSmartCrop, cropAspectRatio: 1 });
    ...
    const iiifComponents = rawIIIFUrl.match(
      /.org\/([0-9]+).jpg\/([^/]+)\/([^/]+)\/0\/default.jpg/
    );
    const iiifSize = iiifComponents[3];
    const iiifUrl = rawIIIFUrl.replace(
      iiifSize,
      cropSize === "full" ? cropSize : `${cropSize},`
    );

    return (
      <div>
        {iiifUrl && (
          <details open={Boolean(currentRegion)}>
            <summary>
              <button
                onClick={getCropFromZoomer}
                disabled={isLoading}
                style={buttonStyles}
              >
                📷 Save detail
              </button>{" "}
              {!currentRegion || isSquare || (
                <button onClick={setSquareSmartCrop} style={buttonStyles}>
                  ✂️ Smart Crop
                </button>
              )}
            </summary>
            <figure style={{ margin: 0 }}>
              ...
              <figcaption>
                  ...
                  <div>
                    <p>
                      {[400, 800, 1200, "full"].map((size) => {
                        const sizeText = isNaN(size)
                          ? "full size"
                          : `${size}px`;
                        return cropSize === size ? (
                          <span key={size} style={{ padding: "0px 7px" }}>
                            {sizeText}
                          </span>
                        ) : (
                          <button
                            key={size}
                            onClick={() =>
                              this.setState({ cropSize: size, isLoading: true })
                            }
                            style={buttonStyles}
                          >
                            {sizeText}
                    ...
                    <label style={{ display: "block" }}>
                      <input
                        ...
                        type="radio"
                        checked={
                          cropAspectRatio !== 1 && cropAspectRatio !== 2 / 3
                        }
                ...
                <p>
                  Zoom in on the left to the detail you'd like to save. Click
                  'Save detail' and wait until the image updates. Right click
                  the image to 'save image as' or copy link, or click the image
