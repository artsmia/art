...
var IMAGE_COUNT = discoverArtworks.IMAGE_COUNT;
...
function uniqueArtworks(artworks) {
  ...
  return (artworks || []).filter(function (art) {
    var id = art && art.id;
    if (!id || seen[id] || !discoverArtworks.hasValidImage(art)) return false;
    seen[id] = true;
    return true;
  });
}
...
function prefetchedArtworks(data) {
  return uniqueArtworks(
    discoverArtworks.getArtworks((data || {}).discoverCarousel)
  ).slice(0, IMAGE_COUNT);
}

var DiscoverCarousel = React.createClass({
  getInitialState() {
  ...
  fetchHighlightArtworks(index, direction) {
    ...
    return discoverArtworks.fetchUniqueArtworksForHighlight(slug).then(
      function (artworks) {
  ...
  slideAnimationClasses() {
    ...
    if (direction > 0) {
      return {
        exit: "discover-carousel__slide-layer--exit-up",
        enter: "discover-carousel__slide-layer--enter-up",
      };
    }
    if (direction < 0) {
      return {
        exit: "discover-carousel__slide-layer--exit-down",
        enter: "discover-carousel__slide-layer--enter-down",
      };
    }
    return { exit: "", enter: "" };
  },
  ...
  renderHeadingValue(slug) {
    ...
    return (
      <span className="discover-carousel__heading-value-wrap" aria-live="polite">
        {previousLabel ? (
          <span
            className={
              "discover-carousel__heading-value discover-carousel__slide-layer " +
              anim.exit
            }
          >
            {previousLabel}
        ...
        <a
          href={searchPath}
          className={
            "discover-carousel__heading-value discover-carousel__heading-link discover-carousel__slide-layer " +
            anim.enter
          }
        >
          {highlightLabel(slug)}
  ...
  renderImageCard(art, isMain) {
    ...
    return (
      <Link
        className="discover-carousel__image-card"
        to="artwork"
        params={{ id: id }}
      >
        <img
          ...
          alt={art.title || ""}
          className="discover-carousel__image"
        />
      </Link>
    );
  },
  ...
  renderImageSlot(art, previousArt, isMain, anim) {
    return (
      <div
        className="discover-carousel__image-slot"
        key={sanitizeArtworkId(art.id)}
      >
        {previousArt ? (
          <div className={"discover-carousel__image-layer " + anim.exit}>
            {this.renderImageCard(previousArt, isMain)}
          </div>
        ) : null}
        <div className={"discover-carousel__image-layer " + anim.enter}>
  ...
  render() {
    ...
    return (
      <section className="discover-carousel">
        <div className="discover-carousel__head">
          <h2 className="discover-carousel__heading discover-carousel__heading--top">
            <span className="discover-carousel__heading-label">Discover</span>{" "}
            {this.renderHeadingValue(slug)}
          </h2>
        </div>

        <div className="discover-carousel__mosaic">
          <div className="discover-carousel__mosaic-main">
          ...
          <div className="discover-carousel__mosaic-grid">
        ...
        <div className="discover-carousel__controls">
          <div className="discover-carousel__nav">
            <button
              className="discover-carousel__nav-button"
              type="button"
