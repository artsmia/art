        ...
        return Exhibition.fetchData.exhibition(params).then((exhibition) => {
          ...
          return rest(searchUrl)
            ...
            .then((json) => {
              ...
              const loanExhibitionAllowlist = [
                2802, 2778, 2851, 2854, 2830, 2856,
              ];
              const exhibitionAllowlisted =
                loanExhibitionAllowlist.indexOf(id) > -1;
              const publishableArtworks = exhibitionAllowlisted
                ? json.hits.hits.filter((data) => data.found) // some exhibitions list IDs not in ES?
                : json.hits.hits.filter(
                    ({ _source: s }) => s && s.public_access == "1"
                  );

              const ids = publishableArtworks.map((hit) => hit._id);

              const publicAccessResults = {
                hits: {
                  total: publishableArtworks.length,
                  hits: publishableArtworks,
                },
              };
              return {
                ...publicAccessResults,
          ...
          meta={[
            {
              property: "og:title",
              content:
                exhibition.exhibition_title + " ^ Minneapolis Institute of Art",
            },
            {
              property: "og:description",
              content: `An exhibition at the Minneapolis Institute of Art. ${exhibition.display_date}`,
            },
          ]}
        />
      </div>
