      ...
      <div onDoubleClick={this.handleDoubleClick}>
        <p className="tombstone">
          <Peek
            facet="medium"
            tag="span"
            highlightedValue={highlight("medium")}
        ...
        {this.state.showLabels && (
          <CopyableLabel art={art} onClose={this.handleDoubleClick} />
        )}
        <p className="gifted">
          <Peek
            facet="creditline"
            tag="span"
            highlightedValue={highlight("creditline")}
            showPeeks={this.props.showPeeks}
          >
            {art.creditline}
          </Peek>
...
var LinkBar = React.createClass({
  ...
  getInitialState() {
    const showDownload =
      this.props.art.rights_type !== "Permission Denied" ||
    ...
    var actions = Object.keys(this.props.actions)
      ...
      .filter(
        (action) =>
          !(
            action.key == "download" &&
            (art.image == "invalid" || art.rights_type === "Permission Denied")
          )
      );
  ...
  showShare() {
    ...
    var facebookURL = `https://www.facebook.com/sharer/sharer.php?u=${encodeURI(
      this.getUrl()
    )}`;
    var twitterURL = `https://twitter.com/intent/tweet?url=${encodeURI(
      this.getUrl()
    )}`;
    var emailURL = `mailto:?subject=${art.title}&body=${encodeURI(
      this.getUrl()
    )}`;
      ...
      <div className="share">
        <div className="social">
          <a title="Share via email" href={emailURL} target="_blank">
            <span className="material-icons" style={{ color: "#232323" }}>
              email
            </span>
          </a>
        </div>
...
var Figure = React.createClass({
  render() {
    ...
    var classes = classnames(className, {
      validImage: art.image === "valid" && art.image_width > 0,
    });
