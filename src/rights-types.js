  ...
  ...require("rightsstatements").map((rs) => {
    rs.definition = rs.definition
      .replace(
        ...
        "The copyright and related rights status of this Item has not been evaluated by the Minneapolis Institute of Art (Mia)"
      )
      .replace(
      ...
      .replace(
        /The organization that has made the Item available/i,
        "Minneapolis Institute of Art (Mia)"
      ) // TODO link 'contact us' to collectionsdata@artsmia.org
...
const miaLegacyRightsTypes = {
  "Copyright Protected":
    "Artwork is copyright protected. Permission from the rights holder named here may be required for use. A copyright statement and credit line must accompany image.",
  "Full Permission":
    "Artwork is copyright protected. Mia holds a non-exclusive license to reproduce images from the rights holder named here. A copyright statement and credit line must accompany image.",
  "Need Permission":
    "Copyright protection is assumed for this artwork. Object rights status has not yet been determined or assigned.",
  "No Known Copyright Restrictions":
    "Artwork may be protected by some other intellectual property, or may be considered ineligible for copyright protection (ex. useful articles). Alternately, artwork may be in the public domain in the United States, but the public domain status has not or cannot be clearly determined.",
  "Non-commercial Use":
    "Artwork is copyright protected. Mia holds a non-exclusive license to reproduce images of this artwork, for limited non-commercial uses only, from the rights holder named here. A copyright statement and credit line must accompany image.",
  "Orphaned Work":
    "Artwork may be subject to copyright protection under the terms of current US copyright law. Mia has been unable to identify the creator and/or rights holder. The statement “Copyright of the artist, artist’s estate, or assignees” should accompany image.",
  "Public Domain":
    "Artwork is in the public domain. Images are available for any use.",
  "Permission Denied":
    "Artwork is copyright protected. Mia has contacted the rights holder and the rights holder denied Mia any use rights.",
};
...
const _rightsStatements = Object.keys(miaLegacyRightsTypes).map(
  (legacyName) => {
    const legacyExplanation = miaLegacyRightsTypes[legacyName];

    const legacyToStand = legacyToStandardizedStmt[legacyName];
    const rightsStmt = rightsstatements.find(
      (stmt) => stmt.label === legacyToStand
    );
...
function RightsStatementIcon({ statement, color, style, ...props }) {
  ...
  var iconURL =
    statement.identifier === "CC-PDM"
      ? `https://upload.wikimedia.org/wikipedia/commons/a/af/Cc-public_domain_mark.svg`
      : `https://rightsstatements.org/files/icons/${shortId}.Icon-Only.${
          color || "white"
        }.svg`;
  var fullIconURL =
    statement.identifier === "CC-PDM"
      ? `https://upload.wikimedia.org/wikipedia/commons/a/af/Cc-public_domain_mark.svg`
      : `https://rightsstatements.org/files/buttons/${statement.identifier}.${
          color || "white"
        }.svg`;
...
module.exports = {
  ...
  findByName: (name) => {
    ...
    const rights = rightsstatements.find(
      (st) =>
        st.label === name ||
        st.legacyName === name ||
        st.identifier === mappedRightsID
    );
