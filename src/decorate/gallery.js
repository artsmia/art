...
var GalleryDecorator = React.createClass({
  ...
  componentWillReceiveProps(nextProps) {
    var dontUpdate =
      this.props.gallery == nextProps.gallery &&
  ...
  render() {
    ...
    return (
      <div style={{ clear: "both" }} className="decorator d-gallery">
        <div>
          <Map {...mapProps} />
        </div>
        {(panel || galleryInfo) && (
          <div className="info">
            ...
            {galleryInfo && (
              <p>
                <strong>{galleryInfo.title}</strong>
              </p>
            )}
            {!showFullInfo && (
              <Link to="gallery" params={{ gallery: number }}>
                More info
              </Link>
            )}
          </div>
  ...
  nextPrevLinks() {
    ...
    var linkParams = (adjacentGallery) =>
      this.props.showFullGalleryInfo
  ...
  fetchData(number) {
    rest(galleryPanelUrl(number || this.state.number)).then((result) => {
      this.setState({
...
var NotOnViewGalleryDecorator = React.createClass({
  render() {
    var message =
      'Very few of the artworks at Mia can be "on view" at any point in time.\n\n' +
