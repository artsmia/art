...
var ArtistDecorator = React.createClass({
  ...
  render() {
    ...
    return (
      <div
        style={{ maxWidth: "47em", margin: "0 auto", color: "white" }}
        key={this.props.terms}
      >
        {thumbnail && (
          <img
            src={thumbnail}
            style={{ float: "right", margin: "0 0 1em 1em", maxHeight: 200 }}
        ...
        {extract ||
          (artistDescription && (
            <p style={{ color: "white" }}>{extract || artistDescription}</p>
          ))}
        <p>
          <a href={`/people/${artist.id}`}>More info &rarr;</a>
        </p>
      </div>
    );
  },
