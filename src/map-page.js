...
var MapPage = React.createClass({
          ...
          {gallery && (
            <div>
              <h3 style={{ textAlign: "center" }}>
                {galleryInfo.id} — {galleryInfo.title}
              </h3>
              <Peek
                key={`preview-${gallery}`}
                q={`room:G${gallery}`}
  ...
  listGalleries() {
    var skipGalleries = [
      111, 112, 113, 114, 267, 268, 269, 270, 271, 272, 273, 274,
    ];
    ...
    return (
      <ul id="map-and-gallery-list" style={{ margin: "1em" }}>
        {gs.map((g) => (
          <GalleryPeek key={g.id} g={g} style={{ minHeight: "78px" }} />
        ))}
      </ul>
    );
  },
...
var GalleryPeek = React.createClass({
  render() {
    ...
    var peek = (
      <li style={{ marginTop: !universal ? "1em" : 0 }}>
        <Peek
          q={`room:G${g.id}`}
          linkProps={link}
          showIcon={false}
