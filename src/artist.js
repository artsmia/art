  ...
  statics: {
    fetchData: {
      artist: (params, existingData) => {
        return rest(`${searchEndpoint}/people/${params.id}`).then(
          ({ entity }) => {
    ...
    return (
      <div>
        {!this.props.hideHeader && (
          <div
            style={{ background: "#232323", height: "7em", width: "100%" }}
          />
        )}
        {false && (
          <Search
        ...
        <div
          ...
          style={{ padding: "1em", maxWidth: "47em", margin: "0 auto" }}
        >
          {thumbnail && (
            <img
              src={thumbnail}
              style={{ float: "right", margin: "0 0 1em 1em" }}
              alt={`image of ${artist.name} from wikipedia`}
            />
          )}
          <h2>{artist.name}</h2>
          {extract && (
            <p>
              {extract} <a href={link}>Read more from Wikipedia &rarr;</a>
            </p>
          )}
          {!extract && artistDescription && (
            <p>
              {artistDescription}{" "}
              <a href={`https://tools.wmflabs.org/reasonator/?&q=${artist.q}`}>
                See more from wikidata &rarr;
              </a>
            </p>
          )}
...
const OtherMuseumsArtistPages = ({ artist }) => {
  ...
  const museumMap = {
    aicID: (id) => [
      ...
      `https://www.artic.edu/artists/${id}`,
    ],
    saamID: (id) => [
      "the Smithsonian American Art Museum",
      `http://americanart.si.edu/collections/search/artist/?id=${id}`,
    ],
    momaID: (id) => [
      "the Museum of Modern Art in New York",
      `https://www.moma.org/artists/${id}`,
    ],
  };
  const others = R.values(
    R.evolve(museumMap, R.pick(R.keys(museumMap), artist && artist.wikidata))
  );
  ...
  return others.length === 0 ? (
    ...
    <div style={{ marginTop: "1em" }}>
      <p>
        ...
        {others.map(([orgName, link], index) => (
          <span key={orgName}>
            <a href={link}>{orgName}</a>
