...
var Aggregations = React.createClass({
      ...
      <div id="aggs">
        ...
        >
          ...
          {aggs.map(function (agg) {
            const aggIsActive =
              search.filters && search.filters.match(new RegExp(agg.name, "i"));
            const showAgg = agg.open || aggIsActive;
            if (showAgg)
              return (
                <dl key={agg.name} id={agg.name} style={defListStyle}>
                  <dt
                    style={{ fontWeight: aggIsActive && "bold" }}
                    onClick={toggleAgg(agg)}
                  >
                    {agg.displayName}
                  </dt>
                  {(agg.open || aggIsActive) &&
                    agg.buckets.slice(0, 5).map(function (bucket) {
                      ...
                      const bucketIsActive =
                        search.filters && search.filters.match(filterRegex);
                      ...
                      if (bucket.key)
                        return (
                          <dd
                            key={agg.name + bucket.key}
                            style={{
                              margin: "0",
                              fontWeight: bucketIsActive && "bold",
                              textDecoration: bucketIsActive && "underline",
                            }}
                          >
                            <span className="objectTotal">
                              {
                                <Link
                                  to={
                                    newFilters == ""
            ...
            {aggs.slice(3).map((agg) => {
              return (
                <dd
                  ...
                  style={{
                    background:
                      agg.open || agg.active ? "rgb(35,35,35)" : "white",
                    color: agg.open || agg.active ? "white" : "rgb(35,35,35)",
                  }}
                >
                  {agg.displayName}
                </dd>
  ...
  buildAndSortAggregationsArray(props = this.props) {
    ...
    return _aggs.sort((a, b) => {
      let [_a, _b] = [order.indexOf(a.name), order.indexOf(b.name)].map(
  ...
  linkToCurrentSearchWithSort(newSort) {
    ...
    var newFilters = "";
    var [querySort, querySortDirection] =
      query && query.sort ? query.sort.split("-") : [undefined, undefined];
    var currentSortIsDescending = querySortDirection === "desc";
    var sortIsActive =
      query &&
      (querySort === newSort || (newSort == "relevance" && !querySort));
    ...
    const complementarySortLink = (
      <Link
        to={!params.splat ? "searchResults" : "filteredSearchResults"}
        query={{
          ...newQuery,
          sort: currentSortIsDescending ? newSort : `${newSort}-desc`,
        }}
        title={`Sort in ${
          currentSortIsDescending ? "ascending" : "descending"
        } order`}
        params={params}
      >
    ...
    return sortIsActive && newSort !== "relevance" ? (
      <span>
        {sortLink} {complementarySortLink}
      </span>
    ) : (
      sortLink
    );
