...
var ArtworkPreview = React.createClass({
  ...
  render() {
    ...
    var showHighlights = highlights && (
      <div className="artwork-detail" style={{ marginTop: "1em" }}>
        <hr />
        <ArtworkDetails
          art={art}
          highlights={highlights}
          skipFields={
            !this.props.showDuplicateDetails &&
            "title style artist medium credit accession_number gallery text tags"
          }
          {...this.props}
        />
      </div>
    ...
    var details = showLink ? (
      <Link to="artwork" params={{ id: art.id }}>
        <div className="objects-page-link">
          <div className="objects-page-icon"></div>details
        </div>
      </Link>
    ) : (
      ""
    ...
    return (
      <Artwork.Figure
        art={art}
        className="objects-focus"
        style={style}
        link={showLink}
        ignoreImageStyle={true}
      >
        <div className="art-details preview-header">
          ...
          {art.object_name === "Inspired By Mia" && (
            <InspiredByMiaBanner art={art} />
          )}
          ...
          <Artwork.Tombstone
            art={art}
            {...this.props}
            highlightAccessionNumber={true}
          />
          <h6>
            <Peek
              showPeeks={this.props.showPeeks}
              facet="room"
              highlightedValue={highlight("room")}
              controlValue={art.room}
            >
              {art.room === "Not on View"
                ? art.room
  ...
  displayBio() {
    ...
    var lifeDateWithoutNationality =
      life_date && life_date.replace(nationality + ", ", "");
    var nationAndDates = (
      <span>
        {nationality && (
          <span>
            <Peek facet="nationality" tag="span" showIcon={false}>
              {nationality}
            </Peek>
            ,{" "}
          </span>
        ...
        {lifeDateWithoutNationality && (
          <Peek facet="life_date" tag="span" showIcon={false}>
            {lifeDateWithoutNationality}
          </Peek>
        )}
      </span>
  ...
  render() {
    ...
    return (
      <div>
        ...
        <p>
          This artwork was <Link to="/info/deaccessions">deaccessioned</Link>
          {reason && ` (${reason})`}
  ...
  render() {
    ...
    return (
      <div>
        <strong>LOAN</strong>
        <p>
          This artwork is not part of Mia's collection. Its owner has
          temporarily loaned it to the museum to display.
        </p>
      </div>
    );
  },
...
var InspiredByMiaBanner = React.createClass({
  render() {
    ...
    return (
      <div>
        <p>
          <strong>Inspired By Mia</strong>: This artwork is not part of Mia's
          collection, but was inspired by it. Our collection is here for you in
          person and online. An art museum has walls, but inspiration is
          limitless.{" "}
          <Link
