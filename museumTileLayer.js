...
L.MuseumTileLayer = L.TileLayer.extend({
  ...
  _computeImageAndGridSize: function () {
    // thanks https://github.com/turban/Leaflet.Zoomify
  ...
  onAdd: function (map) {
    ...
    if (window.location.href.match(/localhost|debug/)) {
      // DEBUG
      window.image = this;
  ...
  fitBoundsExactly: function () {
    ...
    var iAR,
      ...
      imageDimensions = ["container is", cAR <= 1, "image is", iAR <= 1]
        .join(" ")
        .replace(/true/g, "tall")
        .replace(/false/g, "wide"),
      zooms = (this.options.zooms =
        iAR < cAR
          ? { fit: c.y / i.y, fill: c.x / i.x }
          : { fit: c.x / i.x, fill: c.y / i.y });

    var zoom = (this.options.minZoom = map.getScaleZoom(
      zooms.fit,
      map.getMaxZoom()
    ));
